...
public class MainPlugin : Plugin<Config, Translation>
{
    ...
    {
        if (Config.PasteEnabled && string.IsNullOrEmpty(Config.PasteKey))
        {
            Log.Error("Missing paste.ee key! RoundReports cannot function without a valid paste.ee key.");
            return;
        }

        if (Config.DiscordEnabled && Config.DiscordWebhooks.IsEmpty())
        {
            if (!Config.SendInConsole)
            {
                Log.Error("Missing Discord webhook, and console sending is disabled. RoundReports cannot function without a Discord webhook and with console sending disabled.");
                return;
            }

            Log.Error($"Missing Discord webhook! RoundReports will still function, but only users with access to the server console/server logs will receive the report link.");
        }
